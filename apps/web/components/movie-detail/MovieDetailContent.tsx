...
interface Props {
   ...
   * モーダル親 (MovieDetailModal / MovieModal) は ModalAdPortal で広告を
   * モーダル枠の外側に分離して描画するため、内側の AdSlot は不要。詳細ページ
   * (/movies/[slug] や女優ページ) からの利用では従来通り内側で描画する。
   */
  hideAd?: boolean;
}

export default function MovieDetailContent({ movie, hideAd = false }: Props) {
      ...
      <div className="mdc-body">
        ...
        {!hideAd && (
          <div className="mdc-ad-bottom">
            <AdSlot zone="mobileBanner300x250" context="page" />
          </div>
        )}
