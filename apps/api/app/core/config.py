...
class Settings(BaseSettings):
    ...
    # ─────────────────────────────────────────────
    # リバースプロキシ (client IP 抽出) の信頼設定
    # ─────────────────────────────────────────────
    # レートリミット等で使う client IP を X-Forwarded-For から取り出す際に
    # 「信頼できるリバースプロキシの段数」を指定する。攻撃者は XFF の左端
    # (自称 IP) を自由に詐称できるため、左端を鵜呑みにすると per-IP レート
    # リミットを回避されてしまう。信頼済みプロキシは XFF の右側に実際の
    # peer IP を追記する挙動 (nginx proxy_add_x_forwarded_for / 各種 LB) を
    # 前提に、右から数えて TRUSTED_PROXY_HOPS 番目 (= 最も外側の信頼プロキシが
    # 観測した接続元) を client IP として採用する。
    #   - 1 (既定): 直前に信頼プロキシが 1 段いる通常構成 (Railway / nginx 1 段)。
    #   - 0        : プロキシ無しで直接公開する構成。XFF 等のヘッダを一切信用せず
    #                request.client.host のみを使う。
    #   - 2 以上   : CDN + 自前 LB のように信頼プロキシが多段の構成。
