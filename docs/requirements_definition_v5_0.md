...
- [ ] **【新規 TODO】サンプル動画 MP4 URL の動的解決サービス（BFF + ハイブリッド方式）**

  **背景・経緯（2026-05-18 検証で判明した事実）:**

  - DMM Affiliate API の `sampleMovieURL.size_720_480` は HTML プレイヤー（iframe）の URL であり、`<video src>` には使えない。
  - 実 MP4 ファイルは `https://cc3001.dmm.co.jp/pv/<token>/<cid><suffix>.mp4` 形式で配信されており、`<token>` 部分はリクエスト毎に変わる動的署名のように見える。
  - 取得には日本 IP が必須（DMM CDN は海外 IP に GeoIP 403）。
  ...
  - 一方で、ファイル名 suffix のバリエーションが想定より多い:
    - 既知: `_mhb_w` / `mhb` / `_dmb_w` / `dmb`（`sync_catalog.py` の決め打ち、`FeedItem.tsx` の `buildSampleUrlCandidates`、`movies.py` の `_SAMPLE_URL_RE` でハンドリング）
    - 未知の実例: `_sm_s`（cid: `nhd19`）。今後さらに別 suffix が出る可能性が高い。
    - 旧形式 `/litevideo/freepv/<a>/<b>/<cid>/<cid><suffix>.mp4` 自体が存在しない作品（新作・特殊作品）も多数存在する。
  - 結果として、現状の「クライアント側で suffix 全試し（あてずっぽう）＋ 学習キャッシュを DB に書き戻す」方式では、`_sm_s` のような未知 suffix を持つ作品が永久に再生不可となる構造的問題が残っている。

  **採用方針: ハイブリッド方式（BFF + クライアント onError フォールバック）**

  「DB に入っている URL でまず再生 → 失敗時のみ動的取得」の二段構え。月 1 cron で全件更新する案も検討したが、新作リリース直後の窓や未知 suffix への耐性を考えると、必要時のみ動的に取得する方式が将来の suffix 追加にも自動追従でき望ましい。
  ...
  **目標アーキテクチャ:**
  ...
  │ - Redis (Upstash) 短期キャッシュ（TTL 1〜6h） │
  │ - キャッシュミスなら AWS Tokyo の解決器を呼ぶ  │
  ...
  │ Lambda Function URL or Fargate Spot           │
  ...
  **このタスクで併せて削除・簡略化できる現状コード:**

  - `apps/jobs/src/sync_catalog.py::_build_sample_mp4_url`（旧形式 `/litevideo/freepv/` 決め打ち生成）→ **削除**
  - `apps/web/components/FeedItem.tsx::buildSampleUrlCandidates` / `switchSuffix` / `parseSampleUrl`（suffix・パディング・数字prefix の全試し）→ **削除**
  - `apps/web/lib/sampleUrlProbe.ts::probeSampleUrls` の並列 `<video>` プローブ → **単純な onError → resolve-mp4 呼出に簡略化**
  - `apps/api/app/api/v1/endpoints/movies.py::_SAMPLE_URL_RE` の suffix 列挙 → **suffix を完全 optional に簡略化**
  - 月 1 全件リフレッシュ用 cron は維持しても良いが、ハイブリッド学習で DB が自己回復するため必須ではなくなる

  **実装サブタスク:**

  - [ ] AWS Tokyo 側の動的解決器を Lambda（chrome-aws-lambda）と Fargate Spot 常駐のどちらにするか性能評価（コールドスタート 3〜8 秒 / 常駐 0 秒の比較）
  - [ ] `apps/jobs` の `extract_mp4_urls.py` のコア関数（`extract_mp4_url`）を解決器の純粋関数として切り出し、Lambda/Fargate から共通利用
  - [ ] AWS Tokyo にデプロイ（Terraform もしくは AWS CDK で IaC 化）
  - [ ] `apps/api` に `GET /movies/{slug}/resolve-mp4` を新規実装（Redis キャッシュ + AWS RPC + DB 書き戻し）
  - [ ] `apps/web` の onError ハンドラを resolve-mp4 呼出に置き換え、既存のあてずっぽうロジックを削除
  - [ ] `_SAMPLE_URL_RE` を suffix optional に緩和し、`reportSampleUrl` の互換性を確認
  - [ ] 旧形式 URL がまだ多く残る場合に備え、初回 backfill バッチを1回流す（資産再利用）
  - [ ] AWS Tokyo の出口 IP に対する DMM のレート制限挙動を観測（Playwright 同時実行数の上限決定）

  **未確定 / 継続調査事項:**

  - `/pv/<token>/...` の token 寿命の上限（現時点で 32 日以上は確定、年単位かどうかは未確認）
  - 既知 suffix を網羅するために、サンプリングで `_sm_s` 以外にどんな suffix があるかを調査（解決器を動かせば自然に判明する）
...
| MP4 URL のトークン失効 | 当初「数時間で失効する署名 URL」と推測していたが、2026-05-18 の実測により少なくとも 32 日は同一 URL で 200 OK を返すことが確認できた（CloudFront `age: 2796484` 秒で実測、CORS 全開放）。一方で suffix（`_sm_s` 等）には未知のバリエーションがあり、決め打ち URL 生成では未対応の動画が永続的に再生不可となる構造的問題が残る | Phase 4 で BFF + クライアント onError フォールバックによる動的解決方式に移行（§6 Phase 4 の新規 TODO 参照）。それまでは月 1 のフルジョブ＋ クライアント学習キャッシュで暫定運用 |
