...
1. **ECR リポジトリ**を 3 つ作成: `short-video-media/api`, `.../jobs-worker`,
   `.../resolver`。
2. **RDS for PostgreSQL 18** をプライベートサブネットに作成 (db.t4g.small 程度から)。
   - Railway / Xserver 側を Postgres 18 系で運用しているため AWS 側も 18 系で揃える
     (`scripts/migrate-from-railway.sh` や `scripts/backup-postgres.sh` の pg_dump
     クライアントメジャーと一致させると pg_dump / pg_restore のバージョン整合で
     ハマらない)。
   - 18 がコンソールで選べない場合は最新の利用可能メジャー (例: 17) を選び、
     VPS 側もそのメジャーへダウングレード再構築 (`docker compose down -v` 後
     `db.image: postgres:17-alpine` に変更) してから移行する。
